...
bool PhysBody::Contains(int x, int y) const
{
	...
	return this->body->GetFixtureList()->GetShape()->TestPoint(this->body->GetTransform(), {PIXEL_TO_METERS(x),PIXEL_TO_METERS(y) });
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	bool hit = body->GetFixtureList()->GetShape()->RayCast(&output, input, body->GetTransform(), 0);
	if (hit) {
